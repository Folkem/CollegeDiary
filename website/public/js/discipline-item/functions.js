function uploadLessons(idDiscipline) {
    return new Promise((resolve, reject) => {
        ...
        ajaxRequest.open('POST', '/php/get/lessons.php');
        ajaxRequest.onload = () => {
            try {
...
function uploadLessonTypes() {
    return new Promise((resolve, reject) => {
        const ajaxRequest = new XMLHttpRequest();
        ajaxRequest.open('POST', '/php/get/lesson-types.php');
        ajaxRequest.onload = () => {
            try {
                const responseObject = JSON.parse(ajaxRequest.responseText);
                resolve(responseObject);
...
function uploadGrades(idDiscipline, idStudent = -1) {
    return new Promise((resolve, reject) => {
        ...
        const ajaxRequest = new XMLHttpRequest();
        ajaxRequest.open('POST', '/php/get/grades.php');
        ajaxRequest.onload = () => {
            try {
                const responseObject = JSON.parse(ajaxRequest.responseText);
                resolve(responseObject);
...
function uploadHomework(idDiscipline) {
    return new Promise((resolve, reject) => {
        ...
        const ajaxRequest = new XMLHttpRequest();
        ajaxRequest.open('POST', '/php/get/homework.php');
        ajaxRequest.onload = () => {
            try {
                const responseObject = JSON.parse(ajaxRequest.responseText);
                resolve(responseObject);
